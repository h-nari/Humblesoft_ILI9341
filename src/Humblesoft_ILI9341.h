...
class Humblesoft_ILI9341 : public Humblesoft_GFX {
 ...
 public:
  ...
  inline void spi_begin(void) {
    ...
    m_spi->beginTransaction(SPISettings(40000000, MSBFIRST, SPI_MODE0));
#else
    m_spi->beginTransaction(SPISettings(24000000, MSBFIRST, SPI_MODE0));
#endif
  }
  ...
  inline void spi_write(uint8_t c) { m_spi->transfer(c); }
...
#elif defined(ESP32) && 0
#define GPIO_OUT_W1TS ESP_REG(0x60004008)
#define GPIO_OUT_W1TC ESP_REG(0x6000400C)
  inline void cs_active(void) { GPIO_OUT_W1TC = m_cs_mask; };
  inline void cs_idle(void) { GPIO_OUT_W1TS = m_cs_mask; };
...
#error neither ESP8266 nor ESP32
  ...
  inline void dc_data(void) { digitalWrite(m_dc, HIGH); };
#endif
  inline uint8_t spi_read(void) { return m_spi->transfer(0x00); };
  inline void pixel_write(uint16_t color) {
    if (m_pbi > PIXEL_BUF_SIZE - 2) pixel_flush();
  ...
  void drawFontxGlyph(const uint8_t *glyph, uint8_t w, uint8_t h, int16_t cx,
                      int16_t cy, uint8_t textsize_x, uint8_t textsize_y,
                      boolean wrap, uint16_t textcolor,
                      uint16_t textbgcolor) override;
};
